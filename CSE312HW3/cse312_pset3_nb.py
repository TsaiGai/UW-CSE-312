...
class NaiveBayes():
    ...
    def fit(self, train_hams:List[str], train_spams:List[str]):
        ...
        def get_counts(filenames:List[str]) -> Dict[str, int]:
            ...
            """
            # sets a new dictionary of word counts
            word_counts = {}
            ...
            for filename in filenames:
                ...
                # adds the appropriate counts for each word
                for word in words:
        ...
        # keeps track of the number of ham emails
        for ham in train_hams:
            self.num_train_hams += 1

        # keeps track of the number of spam emails
        for spam in train_spams:
            self.num_train_spams += 1

        # gets the word count of ham and spam emails
    ...
    def predict(self, filename:str) -> str:
        ...
        # calculates the probability of an email being ham or spam
        ...
        for word in words:
            # uses Laplace smoothing to get the number of ham words
            count_ham = self.word_counts_ham.get(word, 0) + 1
            # calculates the probability of a ham word occurring
            p_word_given_ham = np.log((count_ham) / (self.num_train_hams + 2))
            # adds the probability previously calculated to p_ham
            p_ham += p_word_given_ham

            # uses Laplace smoothing to get the number of spam words
            count_spam = self.word_counts_spam.get(word, 0) + 1
            # calculates the probability of a spam word occurring
            p_word_given_spam = np.log((count_spam) / (self.num_train_spams + 2))
            # adds the probability previously calculated to p_spam
            p_spam += p_word_given_spam

        # returns the predicted label of the email
        if p_spam > p_ham:
